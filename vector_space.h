...
class vector_space 
{
    private:
        ...
        node get_random_vector()
        {
            ...
            float stddev = 1.5f; // Define the spread of the distribution
            return {
    ...
    public:
        ...
        vector_space(string filename,int seed):
            filename(filename),
            vector_space_range_low(-100), 
            vector_space_range_high(100),
            loaded_bol(false),
        ...
        vector_space(vector<node>vec_space,std::mt19937 seed):
            vector_space_range_low(-100), 
            vector_space_range_high(100),
            loaded_bol(false),
            filename("vectorSpace.vs"),
            gen(seed)
            {
                for(node node_temp : vec_space)
                {
                    add_vector(node_temp);
                }
            }
        ...
        vector<node*> radius_search(node query_node)// contains refference to the dimension, d = 4
        {
            if(verbose)
            {
                ...
                std::cout << "node: [" << query_node.x<< ", " << query_node.y << ", " << query_node.z << ", " << query_node.s << "] \n";
            }
            vector<node*> total_nodes;
            node temp_node = query_node;
            auto key = hashKey(node {query_node.x,query_node.y,query_node.z,query_node.s});
            ...
            for (float x = -cluster_cell_size; x <= cluster_cell_size; x += cluster_cell_size) 
            {
                for (float y = -cluster_cell_size; y <= cluster_cell_size; y += cluster_cell_size) 
                {
                    for (float z = -cluster_cell_size; z <= cluster_cell_size; z += cluster_cell_size) 
                    {
                        node temp = {query_node.x + x,query_node.y + y,query_node.z + z,1};
                        auto key1 = hashKey(temp);
                        ...
                        node temp2 = {query_node.x + x,query_node.y + y,query_node.z + z,-1};
                        auto key2 = hashKey(temp2);
        ...
        vector<node> copy()
        {
            ...
            for (const auto& pair : vect_map) 
            {
                for (node* node_ptr : pair.second) 
                {
                    all.push_back(*node_ptr);  // dereference and copy the actual node
                }
            }
            return all;
        }
        vector_space mutate(float mutation_rate)
        {
            vector<node> all;
            for (const auto& pair : vect_map) 
            {
                for (node* node_ptr : pair.second) 
                {
                    node node_temp = *node_ptr;
                    node_temp.x *= get_random_number(1.0f, mutation_rate);
                    node_temp.y *= get_random_number(1.0f, mutation_rate);
                    node_temp.z *= get_random_number(1.0f, mutation_rate);
                    all.push_back(node_temp);  // dereference and copy the actual node
                }
            }
            auto vs_temp = vector_space(all,get_seed());
            vs_temp.re_map();
            return vs_temp;
        }
        void check_load()
        {
            ...
            {
                cout << "VS: File " << filename << " could not be found \n";
            }
            return;
        }
